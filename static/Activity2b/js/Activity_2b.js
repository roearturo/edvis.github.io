...
function Activity2bcode(){
    ...
    var ejeXLbl = graph.create('text', [5,-7,'$tiempo$\\ \\ \\ \\$(s)$']);  
...
//-----------------------------Graphplothelp------------------//
    ...
    var points1 =[];
    var color="black";
    var GraphType="Time vs Vertical Distance";       
    var Xselected="Time";
    var Yselected="Vertical Distance"; 
            ...
            graf=graph.create('functiongraph', [function(x){return 0.5*acel*x*x},0,animTime], {visible:true,strokeWidth:2,strokeColor: color});
                punt=graph.create('point', [0,0], {
                  name: function(){
                    ...
                    if(this.X()> 3.95 && this.X()<4.05 ) return "(" + this.X().toFixed(1).toString() + "," + (0.5*acel*16).toFixed(1).toString() +")";                            
                    if(this.X()> 4.95 && this.X()<5.05 ) return "(" + this.X().toFixed(1).toString() + "," + (0.5*acel*25).toFixed(1).toString() +")";                            
                    else 
...
/*      ploteaPuntos(x,y);
  ...
  $('#checkFunction').click(function() { 
    ...
    var f= $("#funcionCuadratica").val();
    var f = f.replace(/x/g,"*"); 
        f = f.replace(/t/g, "x")+"*"+"x^2";    
    console.log(f);
