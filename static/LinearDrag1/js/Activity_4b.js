function Activity4bcode(){
    JXG.Options.board.minimizeReflow='none'   

		
	JXG.Options.text.useMathJax = true;
	var board = JXG.JSXGraph.initBoard('edvi',{
		boundingbox:[-10,60,10,-8], //xmin,ymax,xmax,ymin
	...
	var ejeX0Lbl = board.create('text', [65,-4,'$desplazamiento$\\ \\ \\ \\$(m)$']);  
	...
	var ejeYLbl = boardGraphPlot.create('text', [-2,30,'$desplazamiento$\\ \\ $de$\\ \\  $la$\\ \\  $caja$\\ \\ \\ \\$(m)$'],{cssClass:'rotateText', highlightCssClass:'rotateText'});
    ...
    var ejeX1 = boardGraphPlot.create('axis', [[0,0], [1,0]], {
  	...
  	var ejeX1Lbl = boardGraphPlot.create('text', [4,-8,'$Distancia$\\ \\ $para$\\ \\ $soltar$\\ \\ $la$\\ \\  $caja$\\ \\ \\ \\$(m)$']);  
...
/*	var graph= JXG.JSXGraph.initBoard('graph',{
	...
	var particle=board.create('point',[function(){return ball.x},function(){
      	return ball.y;      
      	}],{name: function() { return "(" + ball.x.toFixed(1) + ","+ ball.y.toFixed(1)+")"  }, withLabel:true, labelColor:'black',label: {offset: [-70,20], fixed:false}});
...
/*      ploteaPuntos(x,y);
  ...
  $('#checkFunction').click(function() { 
    ...
    var f= $("#funcionLineal_a").val();
    	f = f.replace(/x/g,"*"); 
        f = f.replace(/d/g, "x");

    var g= $("#funcionLineal_b").val();
    	g = g.replace(/x/g,"*"); 
        g = g.replace(/d/g, "x");
    ...
    console.log(g);
    console.log(h);
    testGrafica.push(curve = boardGraphPlot.create('functiongraph',[h,0,
                    function(){ 
